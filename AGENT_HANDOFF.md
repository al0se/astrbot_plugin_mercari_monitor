...
`seen_items` 的主键是 `(item_id, keyword)`，它表示“消息平台已经成功接受发送”的已通知事实来源。`pending_notifications` 则是持久化待发送队列。

- `订阅`：当前结果进入 `seen_items`，作为基线。
- `搜索`：只返回结果，不落库。
- `刷新`：只把未见商品加入待发送队列，并立即尝试发送；消息平台成功接受后才转入 `seen_items`。
- 定时检查：先将未见商品加入待发送队列，再发送；失败的消息保留在队列，后续整点重试。没有新品时不发送“空提醒”。
...
待发送队列会在 `context.send_message()` 成功返回后，才将对应商品写入 `seen_items`。若平台调用抛异常，商品保留在队列；下一次整点或手动刷新会再次尝试。进程恰好在“平台接受发送”和“写入已见”之间崩溃时，最多会造成重复推送，不会漏推。
